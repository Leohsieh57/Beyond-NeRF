...
std::map<ros::Time, int> stamp_to_key_map;
...
int ensureKeyExists(const ros::Time &stamp, 
    gtsam::Values &values, const gtsam::Pose3 &defaultPose)
{
    auto mit = stamp_to_key_map.find(stamp);
    if (mit == stamp_to_key_map.end())
    {
        int key = stamp_to_key_map.size();
        stamp_to_key_map[stamp] = key;
        values.insert(gtsam::Symbol('x', key), defaultPose);
        key_to_stamp_map[key] = stamp; // Store the ROS timestamp for this key
        return key;
    }
    else
    {
...
void EdgeCallBack(const bnerf_msgs::GraphEdgeCollection::ConstPtr &msg)
{
    ...
    auto pointNoiseModel = gtsam::noiseModel::Diagonal::Variances((gtsam::Vector(3) << 0.1, 0.1, 0.1).finished());
    if (!outFile.is_open())
    {
    ...
    for (const auto &binary_edge : msg->binary_edges)
    {
        ...
        if (closest_unary_edge_start && closest_unary_edge_end)
        {
            ...
            auto pt1 = bnerf::convert<bnerf::Vec3d>(closest_unary_edge_start->mean);
            auto pt2 = bnerf::convert<bnerf::Vec3d>(closest_unary_edge_end->mean);

            int key_start = ensureKeyExists(binary_edge.start_stamp, localInitial, gtsam::Pose3(gtsam::Rot3(), pt1));
            int key_end = ensureKeyExists(binary_edge.end_stamp, localInitial, gtsam::Pose3(gtsam::Rot3(), pt2));

            LOG(INFO) << key_start << ", " << key_end;

            gtsam::Symbol key1('x', key_start);
            gtsam::Symbol key2('x', key_end);

            localGraph.add(gtsam::GPSFactor(key1, pt1, pointNoiseModel));

            localGraph.add(gtsam::GPSFactor(key2, pt2, pointNoiseModel));
        ...
        // outFile << x << "," << y << "," << z << "\n";
            ...
            auto trans = bnerf::convert<bnerf::SE3d>(binary_edge.mean);
            localGraph.add(gtsam::BetweenFactor<gtsam::Pose3>(
                key1, key2, gtsam::Pose3(trans.matrix()), poseNoiseModel));
        }
    }
